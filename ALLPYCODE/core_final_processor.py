...
# ==================== DxAplayerLog 重點事件解析 ====================
_IMPORTANT_EV = {
    # 避免與字幕底色衝突(FFFFCC/D4E6B7/FFDAB9/DDEBF7)，改用極淡馬卡龍色
    # -------- 替換/導入播出單 (最重點操作) --------
    'Replace Playing List Old Start': ('⛔ 替換播出單(舊)', 'FF99CC'),  # 亮粉色 (極顯眼)
    'Replace Playing List New Start': ('✅ 替換播出單(新)', 'CCFF99'),  # 亮綠色 (極顯眼)
    '發送節目單':                      ('📋 發送節目單',   'FFF4EB'),  # 極淡珊瑚
    '發送的目前單':                    ('📋 發送節目單',   'FFF4EB'),  # 同上
    'Recv HeartBeat Update Play List Msg': ('📋 更新播放列表', 'FFF4EB'),
    '新增節目單':                      ('📋 新增節目單',   'FFF9F2'),  # 極極淡珊瑚
    # -------- 人工介入播出操作 ----------
    '編輯當前節目':                    ('✏ 編輯當前節目', 'F3E5F5'),  # 極淡薰衣草紫
    '往播出列表拖入素材':              ('➕ 拖入素材',      'E8F5E9'),  # 極淡薄荷綠
    '貼上節目':                        ('📌 貼上節目',      'E8F5E9'),  # 極淡薄荷綠
    '播出刪除節目':                    ('🗑 刪除節目',      'FFEBEE'),  # 極淡玫瑰粉
    '刪除已播節目':                    ('🗑 刪除已播節目', 'F3E5F5'),  # 極淡紫色
    # -------- 系統控制/跳轉運行 (重點追蹤) --------------
    '完全接管':                        ('🔴 完全接管',      'FFEBEE'),  # 極淡粉
    '鎖定':                            ('🔒 鎖定',          'E3F2FD'),  # 極淡藍
    '解鎖':                            ('🔓 解鎖',          'E3F2FD'),  # 極淡藍
    '使用者登錄':                      ('👤 使用者登錄', 'F1F8E9'),  # 極淡水晶綠
    '系統運行記錄':                    ('⚙ 系統運行記錄', 'F5F5F5'),  # 淺灰
    '自動刪除已播單':                  ('🗑 自動刪除已播', 'F5F5F5'),  # 淺灰
    # -------- 素材作業 ------------------
    '素材檢查':                        ('🔍 素材檢查',   'E1F5FE'),  # 淡藍
    '素材檢查發現錯誤':                ('⚠ 素材錯誤',   'FFEBEE'),  # 極淡玫瑰粉
}
...
def _clean_ev_info(ev_info: str) -> str:
    ...
    # 1.5 針對「切換PGM」或「跳到」保留關鍵字
    if '切換PGM' in s or '跳到' in s:
        return s.strip()
    # 2. 移除標籤（節目名:/拖入素材名:）
    s = _re.sub(r'^節目名:', '', s)
    s = _re.sub(r'^拖入素材名:', '', s)
