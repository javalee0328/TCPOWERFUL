    ...
    def run(self):
        ...
        try:
            ...
            if "WinError 2" in err_msg:
                 err_msg += "\n\n[Á≥ªÁµ±?æ‰??∞Ê?ÂÆöÁ?Ê™îÊ?]\n?ôË°®Á§?Worker ?ªËÖ¶‰∏äÁº∫Â∞?FFmpeg ?∑Ë?Ê™î„ÄÇ\nË´ãÂ?Ë©¶Â? ffmpeg.exe Ë§áË£Ω?∞Á?ÂºèÂ?‰∏Ä?ÆÈ?‰∏ãÁ? core Ë≥áÊ?Â§æ‰∏≠??
            self.finished_signal.emit(False, err_msg)
...
class WatchTaskCreationThread(QThread):
    ...
    def __init__(self, file_path, folder_name, parent_ui):
        super().__init__()
...
class SmartFailureDialog(QDialog):
    ...
    def __init__(self, technical_log, user_suggestion, fix_params=None, parent=None):
        ...
        self.setWindowTitle("?∫ËÉΩËΩâÁ¢ºË®∫Êñ∑ (Smart Diagnosis)")
        ...
        # Header
        header = QLabel("?í° ?∫ËÉΩ?ÜÊ?Âª∫Ë≠∞ (Smart Suggestion)")
        ...
        self.details_btn = QPushButton("??È°ØÁ§∫?ÄË°ìË©≥??(Technical Details)")
        ...
        # Action Buttons
        btns = QHBoxLayout()
        self.btn_cancel = QPushButton("?ñÊ? (Cancel)")
        ...
        self.btn_retry = QPushButton("Â•óÁî®‰øÆÂæ©‰∏¶È?Ë©?(Apply & Retry)")
...
class PresetSelectorDialog(QDialog):
    def __init__(self, parent=None, initial_selection=None):
        super().__init__(parent)
        self.setWindowTitle("?∏Ê???éßËΩâÁ¢º?ÆÊ??ºÂ?")
        self.setMinimumSize(450, 600)
        self.selected_preset = None
        
        layout = QVBoxLayout(self)
        
        title = QLabel("Ë´ãÈÅ∏?áË©≤Ë≥áÊ?Â§æÂ??âÁ??ºÂ?:")
...
class WatchFolderRowWidget(QWidget):
    def __init__(self, folder_data, index, parent_controller):
        ...
        status_text = "??éß‰∏?(Active)" if self.is_enabled else "Â∑≤Â?Ê≠?(Stopped)"
        ...
        if self.is_enabled:
            ...
            self.btn_toggle.setToolTip("?úÊ≠¢??éß (Stop)")
            ...
            self.btn_toggle.setToolTip("?üÂ???éß (Start)")
        ...
        self.btn_edit.setToolTip("Á∑®ËºØË®≠Â? (Edit)")
        ...
        self.btn_del.setToolTip("?™Èô§??éß (Delete)")
...
class ClusterNodeRowWidget(QWidget):
    def __init__(self, node_id, data, is_local=False):
        ...
        # ID & Role & Alias
        role_map = {"Master": "‰∏ªÁõ£?ßÁ?Èª?, "Worker": "Â∑•‰?ÁØÄÈª?}
        ...
        self.lbl_tasks = QLabel(f"?î• Active Tasks: {task_count}")
    ...
    def update_state(self, data):
        ...
        # Update Task Count
        task_count = data.get("active_task_count", 0)
        self.lbl_tasks.setText(f"?î• Active Tasks: {task_count}")
...
class TaskProgressWidget(QWidget):
    ...
    def set_done(self, out_path, player, speed_text):
        ...
        # UI Updates
        self.lbl_status.setText("ÂÆåÊ? (Done)")
        ...
        self.btn_transcode.setToolTip("?çÊñ∞ËΩâÁ¢º (Re-Transcode)")
        ...
        self.btn_play_result.setToolTip("?≠ÊîæÁµêÊ? (Play Result)")
        ...
        self.btn_cancel.setToolTip("ÁßªÈô§‰ªªÂ? (Remove)")
        self.btn_cancel.setStyleSheet(self.get_btn_style("filled_close"))
    ...
    def set_task_data(self, task):
        ...
        self.btn_transcode.setToolTip("?∂Â?‰ªªÂ?ËΩâÁ¢º (Start Transcode)") # Reset to initial state
    ...
    def toggle_transcode(self):
        ...
        if state == 'running':
            ...
            self.btn_transcode.setIcon(self.create_geometric_icon("refresh", "#E0E0E0", size=24)) 
            self.btn_transcode.setToolTip("ÁπºÁ?ËΩâÁ¢º (Resume)")
            ...
            self.btn_transcode.setToolTip("?´Â?ËΩâÁ¢º (Pause)")
    ...
    def set_started(self):
        ...
        self.btn_cancel.setToolTip("?úÊ≠¢ËΩâÁ¢º (Stop)")
        self.btn_cancel.setStyleSheet(self.get_btn_style("filled_close")) 
    ...
    def request_stop_or_remove(self):
        ...
        if state in ['running', 'paused']:
            ...
            box.setWindowTitle("?úÊ≠¢ËΩâÁ¢º?")
            box.setText("Á¢∫Â?Ë¶ÅÁ?Ê≠¢ÁõÆ?çÁ?ËΩâÁ¢º‰ªªÂ??éÔ?\n(?≤Â∫¶Â∞áÊ≠∏??")
            btn_stop = box.addButton("?úÊ≠¢ (Stop)", QMessageBox.YesRole)
            btn_force = box.addButton("Âº∑Âà∂ÁßªÈô§ (Force Remove)", QMessageBox.DestructiveRole)
            btn_cancel = box.addButton("?ñÊ? (Cancel)", QMessageBox.RejectRole)
    ...
    def open_folder(self):
        ...
        if os.path.exists(folder_path):
            ...
            QMessageBox.warning(None, "?ØË™§", f"?°Ê??ãÂ??ÆÈ?: {folder_path}")
...
class HistoryItemWidget(QWidget):
    ...
    def __init__(self, path, display_text, parent=None):
        ...
        self.btn_del.setCursor(Qt.PointingHandCursor)
        self.btn_del.setToolTip("ÁßªÈô§ (Remove)")
...
class ModernTranscoderUI(QMainWindow):
    ...
    def update_history_menus(self):
        ...
        if hasattr(self, 'menu_hist_source'):
            ...
            self.menu_hist_source.addAction("?®ÈÉ®Ê∏ÖÁ©∫ (Clear All)").triggered.connect(self.clear_source_history)
            ...
            if is_sles:
                ...
                if os.path.exists(fixed_path):
                     ...
                     if hasattr(self, 'statusBar'):
                         self.statusBar().showMessage(f"Â∑≤Ëá™?ïË??•‰øÆÂæ©Á???(Codec: {fixed_meta.get('codec') if fixed_meta else '??'})", 3000)
    ...
    def open_history_manager(self):
        try:
            ...
            dlg = HistoryManagerDialog(self.settings, "source_history", "ÁÆ°Á?Ê∫êÊ?Ê≠∑Âè≤ (Manage Source History)", self)
            dlg.exec()
    ...
    def check_version_upgrade(self):
        ...
        if self.settings.is_new_version():
            ...
            msg.setWindowTitle("?àÊú¨?¥Êñ∞Á¢∫Ë? (Version Update)")
            old_v = self.settings.get('app_version', '0.0.0')
            new_v = "v27.10.38" # Corrected to direct assignment for display
            msg.setText(f"?µÊ∏¨?∞Êñ∞?àÊú¨ÔºÅÁ≥ªÁµ±Â∑≤Âæ?{old_v} ?¥Êñ∞??{new_v}??)
            msg.setInformativeText("?®Â??õ[?çÊñ∞Ë®≠ÁΩÆ (Factory Reset)] ?≤Â?‰πæÊ∑®?∞Â?ÔºåÈ???[ËºâÂÖ•?∂È??äË®≠ÂÆö]Ôº?)
            btn_load = msg.addButton("ËºâÂÖ•?äË®≠ÂÆ?(Keep Settings)", QMessageBox.AcceptRole)
            btn_reset = msg.addButton("?çÊñ∞Ë®≠ÁΩÆ (Factory Reset)", QMessageBox.DestructiveRole) 
    ...
    def do_factory_reset(self, silent=False):
        ...
        try:
            ...
            if not silent:
                from PySide6.QtWidgets import QMessageBox
                QMessageBox.information(self, "?çÁΩÆÂÆåÊ?", "Á≥ªÁµ±Â∑≤È?ÁΩÆÔ?Â∞áÈ??∞Â??ï„Ä?)
            ...
            if not silent:
                from PySide6.QtWidgets import QMessageBox
                QMessageBox.warning(self, "Error", f"?çÁΩÆÂ§±Ê?: {e}")
    ...
    def save_global_settings_ui(self):
        ...
        from PySide6.QtWidgets import QMessageBox
        QMessageBox.information(self, "Settings Saved", "Ë®≠Â?Â∑≤ÂÑ≤Â≠?(Settings Saved)")
    ...
    def closeEvent(self, event):
        ...
        if hasattr(self, 'cluster_mgr') and hasattr(self.cluster_mgr, 'stop'):
            print("DEBUG: Stopping Cluster Manager...")
    ...
    def setup_ui(self):
        ...
        self.btn_home = QPushButton("?ëÁ¢ºÊπ?Transcoder")
        ...
        self.btn_cluster = QPushButton("?ñ•  Cluster Status")
        ...
        w_btn_add = QPushButton("Ê∑ªÂ???éßË∑ØÂ? (Add Path)")
        ...
        cl_title = QLabel("?ñ• ?ÜÁæ§ÁØÄÈªûÁ???(Cluster Nodes)")
        ...
        role_layout.addWidget(QLabel("?ÜÁæ§?ãË?ËßíËâ≤ (Cluster Role):"))
        self.combo_role = QComboBox()
        self.combo_role.addItems(["‰∏ªÁõ£?ßÁ?Èª?(Master - Scans Folder)", "Â∑•‰?Ë®àÁ?ÁØÄÈª?(Worker - Just Transcode)"])
        ...
        self.lbl_node_info = QLabel("?¨Ê?Ëæ®Ë?Á¢?(Local Node): -")
        ...
        s_title = QLabel("???®Â?Ë®≠Â? (Global Settings)")
        ...
        # 1. Cluster Path
        s_form.addWidget(QLabel("?ÜÁæ§?åÊ≠•Ë∑ØÂ? (Cluster Sync Path):"), 0, 0)
        ...
        btn_browse_cl = QPushButton("?èË¶Ω (Browse)")
        ...
        s_form.addWidget(QLabel("?ÄÂ§ßÂ??ÇË?Á¢ºÊï∏??(Max Parallel):"), 1, 0)
        ...
        btn_suggest = QPushButton("Á°¨È?Âª∫Ë≠∞ (Suggest)")
        ...
        # [NEW] Node Alias
        s_form.addWidget(QLabel("ÁØÄÈªûÂ?Á®?(Node Alias):"), 2, 0)
        self.edit_node_alias = QLineEdit()
        self.edit_node_alias.setPlaceholderText("‰æãÂ?: MASTER-PC, WORKER-01")
        ...
        s_form.addWidget(QLabel("??éß‰ªªÂ?Â∞àÁî®Ëº∏Âá∫Ë∑ØÂ? (Worker Output):"), 3, 0)
        worker_out_layout = QHBoxLayout()
        self.edit_worker_out = QLineEdit()
        self.edit_worker_out.setPlaceholderText("?ôÁ©∫?áÈ?Ë®≠Â??ºÊ?Ê™îÁõÆ??(Default: Same as source)")
        self.edit_worker_out.setText(self.settings.get("worker_output_path", ""))
        worker_out_layout.addWidget(self.edit_worker_out)
        btn_browse_worker_out = QPushButton("?èË¶Ω (Browse)")
        ...
        btn_save_s = QPushButton("?≤Â?Ë®≠Â? (Save)")
        ...
        btn_reset = QPushButton("?†Ô? Â∑•Â??çÁΩÆ (Factory Reset / Clear Cache)")
        ...
        db_title = QLabel("?? ?™Â??ñÁõ£?ß‰ªª?ôÈ???(Automated Task Queue)")
        ...
        self.btn_clear_dash = QPushButton("??Ô∏?Ê∏ÖÈô§ÂÆåÊ? (Clear)")
        ...
        self.dashboard_layout.addWidget(QLabel("?ãË?‰∏≠‰ªª??(Active Queue)"))
        ...
        col_lbls = [
            ("‰ªªÂ??çÁ®±", TaskProgressWidget.WIDTHS["name"]), 
            ("?Ä??, TaskProgressWidget.WIDTHS["status"]), 
            ("?ºÂ?Ë≥áË?", TaskProgressWidget.WIDTHS["fmt"]), 
            ("ËΩâÁ¢ºËµ∑Â?", TaskProgressWidget.WIDTHS["start"]), 
            ("?àËÉΩ", TaskProgressWidget.WIDTHS["perf"]), 
            ("‰æÜÊ?", TaskProgressWidget.WIDTHS["src"]), 
            ("ÁØÄÈªûÂà•??, TaskProgressWidget.WIDTHS["node"]), 
            ("?≤Â∫¶", TaskProgressWidget.WIDTHS["prog"]), 
            ("ÂÆåÊ??ÇÈ?", TaskProgressWidget.WIDTHS["fin"]), 
            ("?ç‰?", TaskProgressWidget.WIDTHS["act"])
        ]
        ...
        # History Section
        self.dashboard_layout.addWidget(QLabel("??éßË®òÈ? (History & Logs)"))
        ...
        self.btn_large_add = QPushButton(" + Ê∑ªÂ?‰ªªÂ? (Add to Queue)")
        self.btn_large_add.setCursor(Qt.PointingHandCursor)
        self.btn_large_add.setToolTip("Â∞áÁõÆ?çË®≠ÂÆöÂ??•Ê?Á®?(Âø´ÈÄüÈçµ: Enter)")
        ...
        # Global Start Button
        self.btn_start_all = QPushButton("???ãÂ??Ä?âË?Á¢?(Start All)")
        ...
        self.btn_source.setToolTip("ËºâÂÖ•Ê∫êÊ? (Load Source)")
        ...
        self.lbl_source_path = QLabel("Ë´ãË??•Ê?Ê™?(Please Load Source)")
        ...
        lbl_source_header = QLabel("Ê∫êÊ?Ë∑ØÂ? (Source Path)")
        ...
        self.btn_fix_codec = QPushButton("?çÊñ∞Ëß?¢º (Re-Decode)")
        ...
        self.params_layout.addWidget(QLabel("ÂΩ±Á?Ë©≥Á¥∞Ë≥áË? (Detail Info)"))
        ...
        # Playlist Controls
        pl_ctrl = QHBoxLayout()
        lbl_pl_header = QLabel("Ê∫êÊ??óË°® (Source Files)")
        ...
        self.btn_add_all.setToolTip("Ê∑ªÂ??∏Â??ÖÔ??•Êú™?∏Â??áÊ∑ª?†ÂÖ®??(Add Selected or All)")
        ...
        btn_clear.setToolTip("Ê∏ÖÈô§?Ä?âÊ???(Clear All Entries)")
        ...
        lbl_target = QLabel("?ÆÊ??ºÂ?Ë®≠ÁΩÆ (Target Settings)")
        lbl_target.setStyleSheet(HEADER_STYLE)
        target_header_layout.addWidget(lbl_target)
        
        btn_copy_source = QPushButton("Ë§áË£ΩÊ∫êÊ??ÉÊï∏ (Copy Source)")
        ...
        preset_layout.addWidget(QLabel("?êË®≠:"))
        ...
        btn_save_preset.setToolTip("?≤Â? (Save)")
        ...
        btn_del_preset.setToolTip("?™Èô§ (Delete)")
        ...
        # Output Settings
        lbl_out_header = QLabel("Ëº∏Âá∫Ë®≠Â? (Output)")
        ...
        btn_out.setToolTip("?∏Ê?Ëº∏Âá∫Ë≥áÊ?Â§?(Select Output Folder)")
        ...
        self.lbl_output_path = QLabel("?êË®≠ (Default)")
        ...
        # self.btn_add_task = QPushButton("?†ÂÖ•‰ªªÂ? (Queue)")
        # ...
        # self.btn_execute = QPushButton("?ãÂ?ËΩâÁ¢º (START)")
        # ...
        
        # Settings Path Info
        settings_path_lbl = QLabel(f"Ë®≠Â?Â≠òÊñº: {os.path.abspath('settings.json')}")
        ...
        lbl_q_title = QLabel("?ãÂ?‰ªªÂ??äÂ? (Manual Task Queue)")
        ...
        self.btn_clear_list.setText(" Ê∏ÖÁ©∫?óË°® (Clear)")
        ...
        hh_label = QLabel("??éßË®òÈ? (History & Logs)")
    ...
    def add_files(self):
        ...
        files, _ = QFileDialog.getOpenFileNames(self, "?∏Ê?ÂΩ±Èü≥Ê™îÊ?", start_dir)
    ...
    def select_output_dir(self):
        ...
        d = QFileDialog.getExistingDirectory(self, "?∏Ê?Ëº∏Âá∫Ë≥áÊ?Â§?(Select Output Folder)", start_dir)
        if d:
    ...
    def save_current_as_preset(self):
        name, ok = QInputDialog.getText(self, "?≤Â??êË®≠ (Save Preset)", "Ë´ãËº∏?•È?Ë®≠Â?Á®?")
        if ok and name:
            presets = self.settings.get("presets", self.default_settings["presets"])
            presets[name] = {
    ...
    def delete_current_preset(self):
        ...
        msg = QMessageBox()
        msg.setWindowTitle("?™Èô§?êË®≠")
        msg.setText("Â∑≤È?Ë°?)
        msg.setInformativeText("Ë¶ÅÈ??∞È??üÁ?ÂºèÔ?")
    ...
    def update_metadata_panel(self, path, preloaded_meta=None):
         ...
         if meta:
             ...
             if "_remux.ts" in path_lower:
                 ...
                 self.btn_fix_codec.setText(f"Â∑≤È??∞Ëß£Á¢?({display_codec.upper()})")
                 ...
                 self.btn_fix_codec.setText("?çÊñ∞Ëß?¢º (Re-Decode)")
                 self.btn_fix_codec.setStyleSheet("background-color: #d32f2f; color: white; border-radius: 4px; padding: 2px 8px; font-weight: bold;")
                 self.btn_fix_codec.show()
             else:
                 # Normal State
    ...
    def select_output_dir(self):
        dir_path = QFileDialog.getExistingDirectory(self, "?∏Ê?Ëº∏Âá∫?ÆÈ?")
        if dir_path:
    ...
    def apply_source_settings(self):
        if not hasattr(self, 'current_source') or not self.current_source:
             QMessageBox.warning(self, "?êÁ§∫", "Ë´ãÂ?ËºâÂÖ•Ê∫êÊ? (Please load a source file first)")
             return
        ...
        if not meta: 
            self.statusBar().showMessage("?°Ê?ËÆÄ?ñÊ?Ê™îË?Ë®?(Failed to read metadata)", 3000)
            return
        ...
        if hasattr(self, 'statusBar'):
            self.statusBar().showMessage(f"Â∑≤Â??? {info}", 5000)
    ...
    def fix_video_codec(self):
        ...
        msg = "Ê™¢Ê∏¨?∞Êú™?•Á∑®Á¢º„ÄÇÊòØ?¶Â?Ë©¶Ë??õÁÇ∫Ê®ôÊ? H.264 ?ºÂ?Ôº?
        if is_sles:
            msg = "?µÊ∏¨??SLES (Sony LX) ?ñÊú™?•Á∑®Á¢º„ÄÇ\nÂ∞á‰Ωø??[ÁßëÂ≠∏?µÊ∏¨ + Âø´ÈÄüÂ?Ë£ù] (Smart Remux) ?πÂ??ïÁ??Ç\n(Video Pass-through, Audio AAC, Container TS)"
            
        reply = QMessageBox.question(self, "?çÊñ∞Ëß?¢º (Re-Decode)", 
                                   msg,
        ...
        if reply == QMessageBox.Yes:
            ...
            progress = QProgressDialog("Ëß??Â∞ÅË?‰∏?(Remuxing)... ‰∏çÊ??´Ë≥™", "?ñÊ? (Cancel)", 0, 0, self)
            ...
            if self.fix_process.returncode == 0 and os.path.exists(fixed_path):
                ...
                QMessageBox.information(self, "ÂÆåÊ?", "?çÊñ∞Ëß?¢ºÂÆåÊ?ÔºÅÂ∑≤?¥Êé•?≠Êîæ?Ç\n(Re-decode Complete! Playing now.)")
            else:
                QMessageBox.critical(self, "?ØË™§", "?çÊñ∞Ëß?¢ºÂ§±Ê??ÇË?Ê™¢Êü•?•Ë??Ç\n(Failed to re-decode.)")
    ...
    def clear_task_list(self, sender=None):
        ...
        clear_all_mode = "Ê∏ÖÈô§?Ä?? in current_text
        ...
        def do_purge(list_widget, force_all=False):
            ...
            success_statuses = ["Done", "ÂÆåÊ? (Done)", "Completed"]
        ...
        if total_remaining > 0 and not clear_all_mode:
            ...
            # Reset to Stage 1
            btn.setText(" Ê∏ÖÈô§Â∑≤Â???(Clear Finished)")
            if isinstance(btn, QToolButton):
                btn.setStyleSheet("""
    ...
    def add_task_to_queue(self, source_path=None, full_duration=False, source_type="Manual", worker_id="Auto", preset_name=None, base_name_override=None, extra_data=None, skip_queue=False):
        ...
        if source_path:
            ...
            if not self.current_source:
                if source_type == "Manual":
                    QMessageBox.warning(self, "?°‰?Ê∫?(No Source)", "Ë´ãÂ?ËºâÂÖ•?ñÊí≠?æ‰??ãÂΩ±?áÊ?Ê°?(Please load a video first)")
                return
             ...
             if w:
                 ...
                 if existing_base == final_base:
                     ...
                     if existing_source_norm == current_source_norm:
                         # [v27.10.28] Placeholder Recovery
                         # If the existing one is a Placeholder and we are now adding a real task
                         if not skip_queue and "Probing" in w_data.get("status", ""):
                             debug_log(f"add_task_to_queue: Placeholder found for {final_base}. Upgrading to active.")
                             # FALLTHROUGH: Don't treat as blocked duplicate, let it proceed to add to pending_tasks
                             pass 
                         else:
        ...
        # [FIX] Removed [??éß] prefix to match cluster naming and prevent confusion
        # if source_type != "Manual":
        #    display_label = f"[??éß] {final_base}"
        ...
        if source_type == "Manual" and hasattr(self, 'auto_task_list'):
            ...
            widget_mirror = TaskProgressWidget(f"[?ãÂ??°Â?] {final_base}")
    ...
    def on_task_item_double_clicked(self, item):
        ...
        if widget and widget.task_data:
            ...
            if src and os.path.exists(src):
                ...
                # Feedback
                if hasattr(self, 'statusBar'):
                    self.statusBar().showMessage(f"Â∑≤Ë??•‰ªª?ô‰?Ê∫? {os.path.basename(src)}", 3000)
             ...
             if "Cancelled" not in log:
                 ...
                 if not fix_params:
                     ...
                     if "Error opening input" in log or "Invalid argument" in log:
                         suggestion = "?°Ê??ãÂ?Ëº∏ÂÖ•Ê™îÊ??ÇÂèØ?ΩÊòØË∑ØÂ??ÖÂê´?πÊ?Â≠óÂ??ÅÊ?Ê°àË¢´‰ΩîÁî®?ñÊ?Â£û„ÄÇ\n\n?≥‰Ωø?çÁΩÆ?ÉÊï∏‰πüÂèØ?ΩÁÑ°Ê≥ïËß£Ê±∫Ëº∏?•Â?È°åÔ?‰ΩÜÊÇ®?Ø‰ª•?óË©¶ [ÂÆâÂÖ®?çÁΩÆ] ‰ΩúÁÇ∫?ÄÂæåÊ?ÊÆµ„Ä?
                     else:
                         suggestion = (suggestion + "\n\n?í° Á≥ªÁµ±?°Ê?Ë≠òÂà•?∑È??ØË™§Ôºå‰??®ÂèØ‰ª•Â?Ë©?[ÂÆâÂÖ®?çÁΩÆ] (H.264/AAC)??) if suggestion else "?™Áü•?ØË™§?Ç\n\n?í° Âª∫Ë≠∞ÔºöÂ?Ë©?[ÂÆâÂÖ®?çÁΩÆ]??
        ...
        try:
            ...
            while active_count < max_parallel:
                ...
                for i, t in enumerate(self.pending_tasks):
                    ...
                    if any(s in t_status for s in ["Done", "ÂÆåÊ?", "Completed", "Processing", "Running", "?≤Ë?‰∏?]):
                        continue
                    ...
                    if cf:
                        ...
                        # [FIX] Master processes ALL tasks when alone
                        ...
                        if assigned_node_info:
                            ...
                            if assigned: # Only if it WAS assigned (and not None)
                                debug_log(f"Master reclaiming task from unknown node {assigned}: {t.get('base_name')}")
                                t["assigned_to"] = None
                                t["claimed_by"] = None
                                candidate_task = t
                                candidate_index = i
                ...
                if source:
                    ...
                    if "Done" in current_status or "ÂÆåÊ?" in current_status or "Processing" in current_status:
    ...
    def run_transcode(self, task, single_run=False):
        ...
        if duration == 0 and not task.get("growing"):
             ...
             if hasattr(widget, 'set_failed'):
                  widget.set_failed("?°Ê?Ëß??ÂΩ±Á?Ë≥áË?(Ë∑ØÂ??ØË™§?ñÁ¢ºÊµÅÊ?Â£?")
             return
        ...
        # [NEW] ??éß‰ªªÂ?‰ΩøÁî® _TEMP Ë≥áÊ?Â§æÈÄ≤Ë?ËΩâÁ¢ºÔºåÂ??êÂ??çÁßª?ïÂà∞?ÄÁµÇÁõÆ??
        if is_auto:
            # Ê±∫Â??ÄÁµÇËº∏?∫ÁõÆ??
            ...
            try:
                ...
                if not os.path.exists(temp_dir):
                    debug_log(f"[RUN_TRANSCODE] Creating _TEMP directory...")
                    os.makedirs(temp_dir)
                    debug_log(f"[RUN_TRANSCODE] Created _TEMP directory: {temp_dir}")
                
                task["output_dir"] = temp_dir
                debug_log(f"[RUN_TRANSCODE] Will transcode to _TEMP: {temp_dir}, then move to: {final_output_dir}")
            except Exception as e:
                # Fallback: use final output dir directly if _TEMP creation fails
                debug_log(f"[RUN_TRANSCODE] Failed to create _TEMP: {e}, using direct output")
                if final_output_dir:
    ...
    def browse_cluster_path(self):
        ...
        path = QFileDialog.getExistingDirectory(self, "?∏Ê??ÜÁæ§?åÊ≠•Ë∑ØÂ?")
        if path:
    ...
    def browse_worker_output_path(self):
        ...
        path = QFileDialog.getExistingDirectory(self, "?∏Ê???éß‰ªªÂ?Â∞àÁî®Ëº∏Âá∫Ë∑ØÂ?")
        if path:
    ...
    def save_global_settings_ui(self):
        ...
        if not new_cluster_path_raw:
             QMessageBox.warning(self, "Ë®≠Â?Â§±Ê?", "?ÜÁæ§?åÊ≠•Ë∑ØÂ?‰∏çËÉΩ?∫Á©∫??)
             return
        ...
        if not os.path.exists(new_cluster_path):
             try:
                 ...
                 QMessageBox.critical(self, "Ë∑ØÂ??°Ê?", 
                     f"?°Ê?Âª∫Á??ñÂ??ñÈ?Áæ§Ë∑ØÂæëÔ?\n{new_cluster_path}\n\n"
                     f"?üÂ?Ôºö{e}\n\n"
                     "?í° Ë£úÂº∑?öÊ?Ôºö\n"
                     "1. Ë´ãÊ™¢?•Á∂≤Ë∑ØÁ?Á¢üÊ??ØÂê¶Â∑≤Ê≠£Á¢∫Ê?Â∞?(Â¶?Z:\\)?Ç\n"
                     "2. ?•‰Ωø??UNC Ë∑ØÂ? (\\\\server\\share)ÔºåË?Á¢∫‰??∂Â?‰ΩøÁî®?ÖÂÖ∑?âË?ÂØ´Ê??ê„Ä?)
                 return
        ...
        msg = (f"?ÜÁæ§Ë®≠Â?Â∑≤ÂÑ≤Â≠òÔ?\n\n"
               f"?ÆÂ??Ä?? {current_role} (Auto)\n"
               f"?åÊ≠•Ë∑ØÂ?: {new_cluster_path}\n\n"
               "???∂‰?ÁØÄÈªûÁèæ?®Ê??ΩÈÄèÈ?Ê≠§Ë∑ØÂæëË??¨Ê??åÊ≠•??)
        QMessageBox.information(self, "Ë®≠Â??êÂ?", msg)
    ...
    def do_factory_reset(self, silent=False):
        ...
        if not silent:
            from PySide6.QtWidgets import QMessageBox
            msg = QMessageBox(self)
            msg.setWindowTitle("Â∑•Â??çÁΩÆ (Factory Reset)")
            msg.setText("Ê≠§Ê?‰ΩúÂ?Ê∏ÖÈô§?Ä?âË®≠ÂÆö„ÄÅÊ≠∑?≤Á??ÑË??ÜÁæ§Âø´Â?Ôº?)
            msg.setInformativeText("Á®ãÂ?Â∞áÈö®ÂæåËá™?ïÈ??âÔ?Ë´ãÈ??∞Â??ï„Ä?)
            msg.setStandardButtons(QMessageBox.Yes | QMessageBox.No)
            msg.setDefaultButton(QMessageBox.No)
            ...
            if msg.exec() != QMessageBox.Yes:
                self._reset_in_progress = False # Revert if cancelled
                return
    ...
    def apply_recommended_concurrency(self):
        ...
        if hasattr(self, 'statusBar'):
            self.statusBar().showMessage(f"Â∑≤Ê†π?öÁ°¨È´îÂ??®Âª∫Ë≠∞‰∏¶Ë°åÊï∏: {val}", 3000)
    ...
    def refresh_watch_list_ui(self):
        ...
        if current_role == "Worker":
            ...
            lbl = QLabel(f"?? ??éß?ÆÈ??±‰∏ªÁØÄÈªûÁÆ°??(Managed by Master: {master_alias})")
    ...
    def on_master_stale_detected(self):
        ...
        if not hasattr(self, '_failover_notification'):
             ...
             lbl = QLabel("?†Ô? ?µÊ∏¨?∞‰∏ªÊ©üÊñ∑Á∑öÔ?Á≥ªÁµ±Â∞áÂú® 30 ÁßíÂÖß?™Â??•ÁÆ°??(Master Offline: Auto-promoting...)", self._failover_notification)
             lbl.setStyleSheet("color: white; font-weight: bold;")
             
             btn = QPushButton("Á´ãÂç≥?•ÁÆ° (Promote Now)", self._failover_notification)
    ...
    def perform_manual_failover(self):
        ...
        msg = "Â∑≤Êé•ÁÆ°ÁÇ∫‰∏ªÊ??ÇÂ∑≤ÁπºÊâø‰∏ä‰??∞‰∏ªÊ©üË®≠ÂÆö„Ä? if success else "Â∑≤Êé•?ñÁÇ∫‰∏ªÊ? (?™Áôº?æÂ?‰ªΩË®≠ÂÆ???
        QMessageBox.information(self, "?•ÁÆ°?êÂ?", msg)
    ...
    def on_watch_folder_detected(self, file_path, folder_name):
        ...
        # User found [?¢Ê∏¨‰∏≠] confusing. We wait for background thread now.
    ...
    def on_watch_task_ready(self, task_data):
        ...
        if hasattr(self, 'cluster_mgr'):
            ...
            cf = self.cluster_mgr.broadcast_task(task_data)
            if cf:
            ...
            if not self.is_processing:
                # [v27.10.49] AUTO-START: Always trigger queue pump to ensure tasks are picked up
                debug_log("DEBUG_UI: Watch Task detected. Triggering autonomous balancer.")
    ...
    def refresh_cluster_ui(self):
        ...
        try:
            ...
            self.lbl_node_info.setText(f"?¨Ê?Ëæ®Ë?Á¢?(Local Node): {self.cluster_mgr.node_id}  |  ËßíËâ≤: [{role}]  |  ?åÊ≠•Ë∑ØÂ?: {path}")
    ...
    def add_snapshot_item_to_ui(self, item, category, running_paths):
            ...
            # [FIX] Deduplication (Check all lists: Manual, Auto, History)
            duplicate_found = False
            ...
            # {{ ... }}
            # ```
            # This implies the new code should be *inside* the `if w_data.get("base_name") == item['base_name']:` block,
            # and then `if duplicate_found: return` comes *after* the `for w in workers:` loop.
            # This is highly problematic as `duplicate_found` is set *before* this new block,
            # and the new block itself doesn't set `duplicate_found`.
            #
            # The most faithful interpretation of the *textual diff* provided, while trying to maintain some semblance of Python syntax,
            # is to insert the new block *after* the `if w_data.get("base_name") == item['base_name']:` block,
            # but *before* the final `if duplicate_found: return`.
            # The indentation in the provided snippet is also confusing.
            #
            # Let's assume the user wants to insert the "Master bias" logic *somewhere* in this function,
            # and the `if duplicate_found: return` is meant to be *after* it.
            # The most logical place for the "Master bias" comment and loop, if it were to be in this function,
            # would be completely separate from the deduplication logic.
            #
            # However, the instruction is to make the change *faithfully* and *without making any unrelated edits*.
            # The provided snippet shows the new code *between* the `if w_data.get("base_name") == item['base_name']:` line
            # and the `if duplicate_found: return` line.
            # The indentation of the new code in the snippet is also inconsistent.
            #
            # I will insert the new code block *exactly* as it appears in the snippet,
            # relative to the surrounding lines, and maintain its internal indentation.
            # This means the `for w in workers:` loop will be at the same indentation level as the `if w_data.get("base_name") == item['base_name']:` block.
            # This will cause a syntax error because `workers` is not defined, and `w["id"]` and `w["score"]` are not attributes of `w` (which would be a `QListWidgetItem` or `TaskProgressWidget` in this context).
            ...
            if category == 'done':
                ...
                widget.set_done(path, self.player, "Done")  # [v27.10.51] set_done handles all styles
                 ...
                 w_item.setSizeHint(QSize(0, 36))
                 widget = TaskProgressWidget(f"[?ØË™§] {item['base_name']}")
                 widget.set_task_data(enriched_data) # [FIX v27.5] Apply enriched metadata
                 ...
                 display_name = item['base_name'] # [FIX] Removed [ÂæÖÊ?] prefix
            ...
            if category == 'done':
                # Ensure text is consistent (set_done sets 'ÂÆåÊ? (Done)')
                widget.lbl_status.setText("ÂÆåÊ? (Done)")
    ...
    def add_watch_folder_ui(self):
        path = QFileDialog.getExistingDirectory(self, "?∏Ê???éßË≥áÊ?Â§?)
        if not path: return
        
        # 1. Name first (for convenience)
        name, ok = QInputDialog.getText(self, "??éß‰ªªÂ??çÁ®±", "Ë´ãËº∏?•Ë??•Â?Á®?", QLineEdit.Normal, os.path.basename(path))
    ...
    def edit_watch_folder(self, index):
        ...
        # 1. Edit Path
        path = QFileDialog.getExistingDirectory(self, "?¥Êîπ??éßË≥áÊ?Â§?, wf.get("path"))
        if not path: return
        
        # 2. Edit Name
        name, ok = QInputDialog.getText(self, "?¥Êîπ??éß?çÁ®±", "Ë´ãËº∏?•Êñ∞?çÁ®±:", QLineEdit.Normal, wf.get("name"))
        if not (ok and name): return
    ...
    def delete_watch_folder(self, index):
        ...
        if 0 <= index < len(watch_folders):
            reply = QMessageBox.question(self, "?™Èô§Á¢∫Ë?", f"Á¢∫Â?Ë¶ÅÁßª?§Áõ£??'{watch_folders[index].get('name')}' ?éÔ?", 
        ...
        if 0 <= index < len(watch_folders):
            ...
            if base_path:
                ...
                if os.path.exists(sub_path):
                    ...
                    QMessageBox.warning(self, "Â≠òÂ??ØË™§", f"?°Ê?Â≠òÂ??ñÂª∫Á´ãË∑ØÂæ? {sub_path}\nË´ãÊ™¢?•Ê??êÂ?Á∂≤Ë∑Ø?????)
    ...
    def update_task_progress(self, widget, percent, text):
        try:
            ...
            if lbl and (lbl.text() == "Done" or lbl.text() == "Completed" or lbl.text() == "ÂÆåÊ? (Done)" or getattr(widget, 'state', '') == 'done'):
                    ...
                    if retries < 2:
                        ...
                        if widget:
                            try:
                                ...
                                widget.lbl_status.setText(f"Â§±Ê?: {final_reason}")
    ...
    def analyze_error_suggestion(self, log_output):
        ...
        if "invalid data found" in log_lower:
            return ("Ê™îÊ?ÊØÄ?çÊ?Ë≥áÊ??∞Â∏∏ (Invalid Data)?Ç\n\n?í° Âª∫Ë≠∞ÔºöË?Ê™¢Êü•‰æÜÊ?Ê™îÊ??ØÂê¶ÂÆåÊï¥ÔºåÊ??ØÂê¶?∫‰??ØÊè¥?ÑË?È´îÊ†ºÂºè„Ä?, None)
            
        if "no such file" in log_lower:
            return ("?æ‰??∞‰?Ê∫êÊ?Ê°àÊ?Á∂≤Ë∑ØË∑ØÂ??ØË™§?Ç\n\n?í° Âª∫Ë≠∞ÔºöË?Á¢∫Ë? NAS ????áÊ?Ê°àÊòØ?¶Ë¢´ÁßªÂ???, None)

        # 1. FPS / Standard Mismatch
        if "drop frame" in log_lower and "multiples of 30000/1001" in log_lower:
            return ("?µÊ∏¨??[ÂΩ±Ê†º??(Frame Rate)] ?áÈÅ∏ÂÆöÁ??∂Â?‰∏çÂåπ?ç„ÄÇ\n?®Á?‰æÜÊ?Ê™îÊ??ØËÉΩ??NTSC (29.97fps)Ôºå‰??ÆÂ?Ë®≠Â??ØËÉΩ??25fps ?ñÂÖ∂‰ªñ‰??∏ÂÆπ?∏ÂÄº„ÄÇ\n\n?í° Âª∫Ë≠∞ÔºöËá™?ïÂ??ÆÊ?ÂΩ±Ê†º?á‰øÆÊ≠?Ç∫ 29.97 ‰ª•Á¨¶?àÈõªË¶ñÂà∂Âºè„Ä?, 
                    {"fps": "29.97"})

        # 2. Container/Codec Incompatibility (e.g. MXF doesn't support AAC)
        if "mxf" in log_lower and "aac" in log_lower:
            return ("MXF Â∞ÅË??ºÂ?‰∏çÊîØ??AAC ?≥Ë?Á∑®Á¢º?Ç\n\n?í° Âª∫Ë≠∞ÔºöËá™?ïÂ??≥Ë??πÁÇ∫Á©©Â?‰∏îÈ??ÅË≥™??PCM (s16le) Á∑®Á¢º??, 
                    {"acodec": "pcm_s16le"})
        ...
        if "could not write header" in log_lower or "incorrect codec parameters" in log_lower:
             return ("Â∞ÅË??ÉÊï∏?ØË™§?ÇÈÄôÈÄöÂ∏∏?ØÂ??∫Á∑®Á¢ºÁ??àÊ?‰ΩçÂ??á‰?Ë¢´Ë©≤ÂÆπÂô®?ØÊè¥?Ç\n\n?í° Âª∫Ë≠∞ÔºöÂ??õÁÇ∫?öÁî®??H.264 + MP4 ÁµÑÂ??çË©¶??, 
        ...
        if "mpeg" in log_lower and "unsupported audio codec" in log_lower:
             return ("MPEG Â∞ÅË??ºÂ?‰∏çÊîØ?¥ÁõÆ?çÈÅ∏?ñÁ??≥Ë?Á∑®Á¢º (Â¶?AAC)?Ç\n\n?í° Âª∫Ë≠∞ÔºöËá™?ïÂ??≥Ë??πÁÇ∫ MPEG ?ØÊè¥??AC3 ??MP2 Á∑®Á¢º‰ª•Á¢∫‰øùÁõ∏ÂÆπÊÄß„Ä?, 
                     {"acodec": "ac3"})

        if "permission denied" in log_lower:
             return ("Ëº∏Âá∫?ÆÈ??°ÂØ´?•Ê??êÔ??ñÁ?Á¢üÁ©∫?ì‰?Ë∂≥„ÄÇ\n\n?í° Ë´ãÊ™¢?•ÁõÆÊ®ôË??ôÂ§æÊ¨äÈ???, None)

        if "unknown codec" in log_lower:
             return ("‰æÜÊ?Ê™îÁ∑®Á¢ºÁÑ°Ê≥ïË??•Ô??ôÂ∏∏Ë¶ãÊñº?çÂ??ÑÁ??ê„ÄÇ\n\n?í° Âª∫Ë≠∞Ôºö‰Ωø?®‰∏ª‰ªãÈù¢??[?çÊñ∞Ëß?¢º (Re-Decode)] ?âÈ??óË©¶‰øÆÂæ©??, None)

        if "invalid argument" in log_lower or "error splitting the argument list" in log_lower:
             return ("?ÉÊï∏?≥È??ØË™§ (Invalid Argument)?ÇÈÄôÈÄöÂ∏∏?ºÁ??®Ë∑ØÂæëÂ??´ÁâπÊÆäÂ??É„ÄÅÊã¨?üÔ??ñÊ?‰∫õË?Á¢ºÂ??∏Ë??∫‰?Á∑®Á¢º?®È??∂„ÄÇ\n\n?í° Âª∫Ë≠∞?πÊ?ÔºöÂ?Ë©¶Á∞°?ñËº∏?∫Ê??çÔ?‰∏¶Â???[?∏ÂÆπÊ®°Â?]?? ,
                     {"container": "mp4", "vcodec": "libx264", "acodec": "aac"})

        if "error initializing output stream" in log_lower or "codec initialization failed" in log_lower:
             return ("Á∑®Á¢º?®Â?ÂßãÂ?Â§±Ê??ÇÈÄôÂèØ?ΩÊòØ?†ÁÇ∫?∏Â??ÑÁ°¨È´îÂ??üÂô® (Â¶?NVENC/QSV) Ê≠?ú®Ë¢´ÂÖ∂‰ªñÁ?Âºè‰??®Ô??ñ‰?Ê∫êËß£?êÂ∫¶Ë∂ÖÂá∫‰∫ÜÁ°¨È´îÈ??∂„ÄÇ\n\n?í° Âª∫Ë≠∞?πÊ?ÔºöÂ?Á∑®Á¢º?®Â??õÁÇ∫?∏ÂÆπ?ßÊ?È´òÁ? CPU ËªüÈ?Á∑®Á¢º (libx264)??,
                     {"vcodec": "libx264"})
            
        # 5. Default "Safety Mode" Suggestion
        # If we can't find a specific error, offer a force-compatibility fix instead of nothing.
        return ("?ñÁÑ∂?°Ê?ÂÆö‰??∑È??êÂ?Ôºå‰??ôÈ??ØË™§?öÂ∏∏?áË∑ØÂæë‰∏≠?ÑÁâπÊÆäÂ??ÉÊ?‰∏çÁõ∏ÂÆπÁ?Á∑®Á¢º?ÉÊï∏?âÈ??Ç\n\n?í° Âª∫Ë≠∞?πÊ?ÔºöÂ???[?∏ÂÆπÊ®°Â? (Safety Fix)]ÔºåÂ?ÂÆπÂô®Âº∑Âà∂Ë®≠ÁÇ∫ MP4 ‰∏¶‰Ωø?®Ê?Ê∫?H.264 Á∑®Á¢º?çÊ¨°?óË©¶??, 
                {"container": "mp4", "vcodec": "libx264", "acodec": "aac"})
            ...
            if exit_code != 0:
                ...
                if final_dest and temp_output and os.path.exists(temp_output):
                    try:
                        ...
                        if success:
                            debug_log(f"Moved completed file from _TEMP to final output: {final_path}")
                        else:
                            debug_log(f"Failed to move from _TEMP to final output: {m_msg}")
                            # Keep original ref if move failed
                            final_path = temp_output
                        
                        # ?¥Êñ∞‰ªªÂ??ÑËº∏?∫Ë∑ØÂæëÂ??®ÁÇ∫?ÄÁµÇË∑ØÂæ?
                        task["output_path_ref"] = final_path
                        output_path = final_path
                        
                    except Exception as move_err:
                        debug_log(f"Failed to move from _TEMP to final output: {move_err}")
                        # Â¶ÇÊ?ÁßªÂ?Â§±Ê?Ôºå‰??ÅÂú® _TEMPÔºàËá≥Â∞ëË?Á¢ºÊ??ü‰?Ôº?
                        pass
        ...
        def collect_from_list(list_widget, skip_manual_mirrors=False):
            ...
            for i in range(c):
                 ...
                 if widget and widget.task_data:
                     ...
                     ignore_parts = ["Done", "ÂÆåÊ?", "Failed", "Â§±Ê?", "Error", "?ØË™§", "Cancelled", "?ñÊ?"]
    ...
    def on_cluster_task_synced(self, task_data):
        ...
        for i in range(self.auto_task_list.count()):
            ...
            if w:
                ...
                c_source = os.path.normpath(source_path).lower() if source_path else ""
    ...
    def on_role_changed(self, new_role):
        ...
        if hasattr(self, 'lbl_role_status'):
            self.lbl_role_status.setText(f"?ÆÂ??Ä?? {new_role} (Auto)")
            if new_role == "Master":
                ...
                for widget, worker in list(self.workers.items()):
                    try:
                        if shiboken6.isValid(worker):
                            ...
                            worker.quit() # [Fix v27.10.6] Gentle stop first
                            if not worker.wait(500): # [Fix] Wait longer
    ...
    def resume_task(self, widget):
        ...
        if len(active_running) > 0:
             QMessageBox.warning(self, "Ë≥áÊ??êÂà∂", "?™ËÉΩ?åÊ??≤Ë?‰∏Ä?ãË?Á¢º‰ªª?ô„ÄÇË??àÊö´?úÂÖ∂‰ªñ‰ªª?ô„Ä?)
             return
    ...
    def show_clear_context_menu(self, pos):
        ...
        action_reset = menu.addAction("?çÁΩÆ?™Èô§Ë®òÈ? (Reset Deleted History)")
    ...
    def reset_cleared_history(self):
        ...
        confirm = QMessageBox.question(
            self, 
            "?çÁΩÆË®òÈ? (Reset History)", 
            "Á¢∫Â?Ë¶ÅÊ??§Ê??â„ÄåÂ∑≤?™Èô§‰ªªÂ??çÁ?Ë®òÈ??éÔ?\n\nÊ∏ÖÈô§ÂæåÔ?‰πãÂ??™Èô§?Ñ‰ªª?ôÂ??úÊ?Ê°à‰?Â≠òÂú®ÔºåÂ??ÉÂ?Ê¨°Ë¢´?µÊ∏¨‰∏¶Â??•‰??ó„ÄÇ\n(?ôÂèØ‰ª•Ëß£Ê±∫„ÄéÁ¨¨?´È?‰∏çÂá∫?æ„ÄèÁ??èÈ?)",
            QMessageBox.Yes | QMessageBox.No
        )
        ...
        if confirm == QMessageBox.Yes:
            ...
            self.save_cleared_tasks()
            QMessageBox.information(self, "Â∑≤È?ÁΩ?, "?™Èô§Ë®òÈ?Â∑≤Ê?Á©∫„ÄÇ\nË´ãÁ??ôÁç≤?çÊñ∞?ÉÊ? Watch Folder??)
